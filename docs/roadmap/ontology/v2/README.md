...
本原型当前只作为 ontology v2 的业务结构草案，不再定义客户端里的 SOP 工作流模块。客户端接入必须收敛到以下事实源：

- `current`：内容制造批次、资料接入 / 输入源、品牌 / IP 知识库、场景库、Prompt 草稿、审核 / 生产交接行动记录、`run-trace` 历史追溯。
- `compat`：旧 `workflowRunId`、`workflow-runs.json` 和 `sop-input` 只用于旧工作区数据追溯，不能承载新业务逻辑。
- `dead`：`WorkflowStore`、`WorkflowEngine`、`WorkflowFeatureModule`、SOP 表单入口、工作流定义维护和 Canvas 编排不再作为当前客户端主线恢复。
...
批次阶段轨道不是最终产品形态，只是流水线的运行骨架。数据入口负责把外部系统、文件和人工输入变成可追溯输入包；知识库负责事实和关系，规则负责门禁，Agent 负责执行，审核负责放行，投放调优负责把创意接入流量和预算，复盘负责让下一批更准。
...
因此，页面上仍然只有一条批次阶段主线，但当前阶段下会展开具体对象、规则、Agent 任务、变量和结构化输出。这样既避免入口罗列，也不会丢掉真实业务细节。
