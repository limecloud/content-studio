...
- 两个不同团队账号都能读取同一个工作区、同一个默认团队知识包、同一批审核任务和同一批执行队列。
- 团队知识包已经确认发布，公开包地址可访问。
...
| --- | --- |
| 顶层结果 | `ok: true`。 |
| API 地址 | 非 localhost / 127.0.0.1，生产默认 `https://api.bugu.run`。 |
...
| 公开包 | `package.reachable: true`，有非本地 `publicUrl`。 |
| 包完整性 | 有 `size > 0` 和 64 位十六进制 `sha256`。 |
| 两账号共享 | actor A / B 看到同一个工作区和同一个非空 revision。 |
| 审核 / 队列清单 | actor A / B 的 `reviewTaskCount`、`executionQueueCount` 一致；如有任务或队列，`reviewTaskIds`、`executionQueueIds` 必须一致。 |
...
| 公开包不可访问 | 检查对象存储、R2 / OSS 公网地址、权限和缓存。 |
| sha256 不一致 | 重新发布团队知识包，不能手改报告。 |
| 报告使用 localhost | 只能作为功能测试证据，不能归档为生产通过。 |
