...
- Content Studio 已落地内容知识地图、审核任务、品牌战情室、生产交接、素材回写、团队知识包导出和 Bugu 同步适配器的本地主链。
...
- Content Studio 生产交接生成的 Prompt 草稿、场景卡或 SOP 运行记录会绑定团队知识包版本，避免下游消费脱离已审核口径；Prompt 工作台手动生成草稿和启动 Prompt 协作时也可以选择已发布团队知识包，草稿和会话都会保留同一版本引用；内容知识地图的团队知识包详情页也可以直接生成带版本引用、覆盖行、来源引用和禁用边界的 Prompt 草稿，不再只是跳转空工作台。
...
- Content Studio 已新增团队共享只读在线验收脚本，要求两组账号 token 同时看到同一团队工作区、同一默认知识包、审核任务、执行队列和行动记录；功能测试用本地 HTTP server 模拟双账号链路。
- 团队共享在线验收已补同清单校验：两账号看到的审核任务数量、执行队列数量、行动记录数量、任务 ID 清单、队列 ID 清单和行动记录 ID 清单必须完整分页拉取且一致；生产归档门禁会拒绝任一清单不一致或清单未完整拉取的报告。
...
- Content Studio 和 Bugu 控制台已补同步冲突逐项合并处理清单：冲突卡片可展开查看本机提交、团队当前内容、建议处理方式和下一步，不自动覆盖团队版本。
...
- Content Studio 内容知识地图页已补团队知识包详情浏览：普通用户可以在当前工作台查看团队版本、文件数、包文件、对象 key、sha256、确认状态和最近版本，并能从已发布版本直接生成 Prompt 工作台草稿；未发布版本只显示恢复路径，不伪装成团队可复用口径。
- Content Studio 内容知识地图矩阵已把证据 ID 转成可读摘要，痛点行会优先展示用户原声或客服记录，承接 prototype 中“评论痛点聚类必须看见真实原话”的要求。
- Content Studio 已补素材库作为内容知识地图构建输入：`AssetReviewStore` 的真实素材审核记录会进入 seed、模型 prompt、`asset-review` 证据、素材验证行、素材驳回复盘行和覆盖摘要；模型调用和本地知识地图都不写入本机素材路径。
...
- Content Studio 品牌战情室资源包已读取矩阵行真实素材引用，执行队列 ready 的 `generate-prompt-draft` 动作会创建真实 Prompt 草稿并回填资源包，`create-scene-card` 动作会创建真实场景卡并回填资源包，`launch-sop-run` 动作会通过 `WorkflowEngine` 启动真实 SOP 并回填资源包，`write-back-material-coverage` 动作会调用素材覆盖回写服务并留下覆盖变更引用；补证据 / 补素材 / 送审动作会创建真实审核任务并可同步到 Bugu；目标树、作战编组和执行队列页的主动作会分别写入 `confirm-objectives`、`confirm-resource-bundles` 和 `sync-execution-queue` 行动记录，执行队列主动作会同步队列状态到团队事实源；行动记录页的复盘结论会写入 `review-action-records` 并同步团队，行动记录导出会生成本机交付文件并写入 `export-action-records` 团队审计记录。执行前发布检查已覆盖平台规则和团队角色权限，Bugu 行动记录追加接口会拒绝只读角色写入。
...
- Content Studio 作战分组已把左侧“品牌战情室、目标树、作战编组、执行队列、行动记录”落成真实直达视图：每个入口都有独立页面标题、当前业务对象、主判断、主动作、交付去向和真实 record 计算的小型图表；目标树“确认目标优先级”、作战编组“保存作战单元”、执行队列“同步执行队列”都落入 IPC、主进程服务、本机 Store 和 Bugu 同步适配器，不再只停留在页面按钮；页面只消费 `BrandCommandCenterRecord`、内容知识地图、队列和行动记录，不补 mock 数据。
...
- Content Studio 真实客户端已补原型第一批落地差距：内容知识地图支持矩阵行下钻查看证据摘录、素材状态、恢复路径和交付去向；行详情可直接把已审核组合交接成真实 Prompt 草稿、场景卡或 SOP 执行，未审核组合会先创建审核任务；详情页可切换 IP 口径、竞品观察、团队知识包、素材回写和高级导出；团队知识包页可生成真实 Prompt 草稿，高级导出可生成本机预览。
...
- 已新增 v1 本地 readiness gate：`npm run content:v1:verify-readiness` 会检查 v1 必需文档、实现文件、在线验收脚本、package scripts、普通用户原型禁用词、完成度审计、文档状态口径、历史状态措辞、三类 current 主事实源、compat 旁路边界和生产报告归档状态；默认本地模式允许缺少生产报告但给出 warning，`-- --require-production-report` 会把缺少真实线上报告视为失败。该 gate 已接入 `npm run verify:local`。
- 生产证据待补：使用真实 R2 / OSS、真实账号和两台设备运行 v1 在线验收总入口并归档报告。
...
| 里程碑 | 目标 | 当前状态 |
| --- | --- | --- |
| v1.0 | 内部 Ontology Store、用户可见内容知识地图、产品卖点矩阵、用户反馈痛点矩阵、人工审核、发布到 PromptDraft。 | Local Verified |
| v1.1 | IP Ontology、竞品观察、素材覆盖回写、审核台解释能力。 | Local Verified |
| v1.2 | 品牌内容作战系统，支持信号雷达 -> 作战目标 -> 资源包 -> 执行队列 -> 行动记录。 | Local Verified |
| v1.3 | Agent Knowledge v0.7.2 导出、可选 answer-ready 层、互操作导出。 | Local Verified |
| v1.4 | Bugu 服务端团队工作区、冲突队列、离线草稿同步和团队 release 消费；按需对接 LimeCore OEM 云底座。 | Local Verified / Production Evidence Pending |

当前 v1 主线的本地实现、功能测试、E2E 和 readiness gate 已闭环；生产完成声明只差真实 Bugu 工作区、真实双账号、真实对象存储公开包和线上验收报告归档。
