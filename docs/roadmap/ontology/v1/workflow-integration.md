...
更新时间：2026-05-29
状态：Draft / First Team Release Consumption Cut

## 1. 设计结论

Ontology v1 是现有模块之间的中间层。它不替代知识库、场景库、Prompt 工作台、SOP 或素材库，而是为这些模块提供结构化、已审核、可追溯的上下文。

当前第一刀已经让团队知识包进入下游消费链路：生产交接生成 Prompt 草稿、场景卡或 SOP 运行记录时会绑定团队知识包版本；素材覆盖已进入待确认补充任务，团队知识包已支持多步骤确认和默认确认模板；内容知识地图矩阵已支持筛选、排序、分页和本批送审；内容知识地图生成会写入可审计流程记录，并新增 v1 在线验收总入口。后续仍需用真实账号、两台设备和生产对象存储归档验收报告。

真实客户端行详情已接入生产交接服务：已审核的矩阵组合可以直接生成 Prompt 草稿、场景卡或 SOP 运行记录；未审核组合会先创建审核任务并进入审核台。若当前工作区没有真实已发布团队知识包，SOP 运行仍保留本机输入、步骤和产物线索，但不能宣称已完成生产团队 release 绑定。
...
| `AssetReviewStore` | 回写素材覆盖、审核结论和表现标签。 | 不把素材表现自动当成产品事实。 |
...
- IP 语言规则和核心立场已进入普通生产前的规则边界；validation 和生产交接 policy 已能拦截常见 IP 漂移表达，后续再升级为更细字段级 `ip-voice-drift` issue。
...
| `create-material-gap-list` | missing-material rows。 | 素材缺口清单。 |
...
- Prompt 工作台和 SOP 默认消费 `KnowledgeRelease`，不直接消费他人本地 draft；当前实现已在 PromptDraft 和 WorkflowRun 中保存团队知识包版本引用。
