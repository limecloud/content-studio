...
当前第一刀已经让团队知识包进入下游消费链路：生产交接生成 Prompt 草稿、场景卡或 SOP 运行记录时会绑定团队知识包版本；素材覆盖已进入待确认补充任务，团队知识包已支持多步骤确认和默认确认模板；内容知识地图矩阵已支持筛选、排序、分页和本批送审，并新增 v1 在线验收总入口。后续仍需用真实账号、两台设备和生产对象存储归档验收报告。
...
- 正式接 Bugu 后每一步都写构建运行记录，不能只保存最终结果。
...
| 卖点 / 场景矩阵 | 卖点 / 痛点 / 人群 / 场景 / 渠道 / 证据 / 素材状态。 | 筛选、排序、分页、本批选择、生成审核任务。 |
