...
- 本地功能测试已通过，覆盖 v1 关键策略、UI 文案门禁、团队共享模拟、知识包导出、在线验收报告归档门禁和两账号同清单校验；UI 文案门禁已经覆盖内容知识地图、审核台、品牌战情室、Prompt 工作台和 SOP 执行页这五个普通用户主路径。
- v1 HTML 原型已补成可交互确认版，普通用户不需要理解 Ontology 也能按业务对象下钻证据、风险、恢复路径和交付去向。
- Content Studio 真实客户端已补齐原型第一批关键承诺：内容知识地图支持矩阵行下钻查看证据、风险、恢复路径和交付去向；IP 口径、竞品观察、素材回写、团队知识包内容和高级导出可在真实页面切换查看；矩阵行可直接交接为真实 Prompt 草稿、场景卡和 SOP 执行链路，未审核组合先进入审核台；作战系统支持从左侧直接进入目标树、作战编组、执行队列和行动记录，并把“确认目标优先级 / 保存作战单元 / 同步执行队列”以及执行队列中的 Prompt 草稿、场景卡、SOP 运行、素材覆盖回写落成真实行动记录、队列同步和 Bugu `content-command-centers` 主事实源快照。
...
- Content Studio 行动记录页已补真实复盘写入和导出交付：复盘结论会追加为 `review-action-records` 行动记录并同步团队，同时在当前品牌战情室生成下一轮信号、复盘目标、资源包、作战单元和补资源 / 审核 / 下一轮 Prompt 队列动作，再同步到 Bugu `content-command-centers` 主事实源；点击“导出行动记录”会生成本机 `manifest.json`、`action-records.md` 和 `action-records.json` 交付文件，文件内容脱敏本机路径，并追加 `export-action-records` 团队审计记录，不再只是 Agent 对话或只读时间线。
...
- Content Studio 已把品牌战情室补素材动作落为真实交付包：执行 `create-material-gap-list` 会生成 `manifest.json`、`material-gap-list.md` 和 `material-gap-list.json`，行动记录写入交付文件引用，UI 展示交付物文件名，Bugu 行动记录同步 payload 会脱敏本机路径。
...
- 品牌战情室队列直出链路已补团队知识包版本边界：`generate-prompt-draft` 和 `launch-sop-run` 会把当前内容地图的已发布团队知识包写入 Prompt 草稿、SOP 运行和行动记录；当前地图没有 release 时不会借用其他项目 release，行动记录页只展示“团队知识包：标题 版本”的业务追溯信息。
...
- Bugu 业务后端已补 `content-knowledge-maps`、`content-build-runs` 和 `content-command-centers` current 服务端事实源：团队内容知识地图快照、覆盖摘要、质量问题、构建运行步骤、品牌作战信号、目标、资源包、作战单元、队列摘要和行动摘要不再只存在于桌面本地 JSON、执行队列或行动记录旁路里；Content Studio 构建完成后会先同步知识地图快照，再同步生成流程摘要和品牌作战系统快照，未登录 Bugu 时仍只保存本机缓存，不伪造团队同步。
- Content Studio 已补 current 事实源读回本机缓存：内容知识地图列表会分页拉取 Bugu `content-knowledge-maps`，生成流程列表会分页拉取 `content-build-runs`，品牌战情室列表会分页拉取 `content-command-centers` 完整快照；团队刷新失败时只保留本机缓存，不把 release、执行队列或行动记录旁路当作完整团队事实源；本机已同步且更新时间不早于团队快照时，会保留本机完整作战结构并只合并团队新增行动记录，防止旧团队快照覆盖复盘生成的信号、目标、资源包、作战单元和队列动作。
...
1. 真实 Bugu 业务后端工作区：`https://api.bugu.run` 返回真实团队内容工作区、团队 revision、知识地图、构建运行、品牌作战系统、审核任务、执行队列、行动记录和默认知识包。
2. 两组真实团队账号：actor A / B 读取同一工作区、同一默认知识包、同一批团队知识包版本、同一批知识地图、同一批构建运行、同一批品牌作战系统、同一批审核任务、同一批执行队列和同一批行动记录。
...
| Playwright E2E | `npm run test:e2e` | 通过。33 个用例全通过，包含视频素材包 SOP 二次运行、审核交接、品牌战情室对话、SOP 表单运行、内容知识地图 v1 真实工作台和业务主链待配置边界。 |
...
| 真实客户端 v1 下钻回归 | `npm run test:e2e -- -g "内容知识地图 v1 真实工作台支持下钻、素材回写和作战入口"`，2026-06-01 | 通过。验证内容知识地图种子数据写入、工作区刷新、生成流程详情页签、矩阵行下钻、真实 Prompt 草稿交接、真实场景卡交接、真实 SOP 运行交接、IP 口径、竞品观察、团队知识包详情、团队知识包详情页直接生成 Prompt 草稿、素材回写、高级导出、本机预览、目标树、作战编组、执行队列和行动记录；高级导出页点击“生成本机预览”后会显示 Agent Knowledge v0.7.2、素材覆盖、JSON-LD / Turtle / RDF/XML、`compiled/prompt-grounding.md`、包大小和 sha256，并能切换点击 `compiled/prompt-grounding.md` 读取地图标题、可复用卖点、用户痛点、场景、规则边界，点击 `assets/material-coverage.json` 读取 `rowId` 和 `materialRefs` 等真实素材覆盖内容；团队知识包 Prompt 草稿包含团队版本、地图标题、覆盖行、来源引用、可复用卖点、禁用边界和节奏等下游变量；目标树点击“确认目标优先级”写入 `confirm-objectives`，作战编组点击“保存作战单元”写入 `confirm-resource-bundles`，执行队列点击“同步执行队列”写入 `sync-execution-queue` 并回填队列同步状态；同一用例会检查 Bugu `content-command-centers` 最新 payload 中的信号、资源包、队列摘要、行动摘要、复盘记录、已交接队列、已回写队列和脱敏交付物引用，防止作战系统退回为只写本机 JSON 或旁路行动记录；素材回写页点击“创建补素材任务”后会进入审核台，并从 preload API 读取到 `material-supplement / needs-material / request-material` 任务；素材库详情页点击覆盖组合的“补这个组合”后同样进入审核台并落真实补素材任务；执行队列页点击“记录交接”后会写入非 `handoff:*` 队列行动记录，分别生成真实 Prompt 草稿、真实场景卡、真实 SOP 运行，并通过素材覆盖回写把队列推进为 `handed-off` / `written-back`；Bugu `content-command-centers` 快照同步校验 `create-scene-card` 的 `sceneCardId`、`launch-sop-run` 的 `workflowRunId`、`write-back-material-coverage` 的 `materialCoverageChangeId`；行动记录页点击“写入复盘记录”会写入 `review-action-records`，并在信号雷达和执行队列中看到复盘信号、复盘补素材动作和待补资源状态，Bugu `content-command-centers` 快照也必须包含对应下一轮信号、复盘目标、资源包、作战单元和 `create-material-gap-list / needs-resource` 队列动作；点击“导出行动记录”会写入 `export-action-records` 审计记录。 |
| 品牌作战系统 current 快照保护 | `npm run test:functional -- --test-name-pattern "品牌战情室团队事实源刷新不会覆盖本机已同步的更新快照"`，2026-05-31 | 通过。验证 `BrandCommandCenterApplicationService.list()` 在本机 `content-command-centers` 快照已同步且更新时间不早于团队旧快照时，保留本机复盘生成的信号、复盘目标、资源包、作战单元和补素材队列，只合并团队新增行动记录，防止 `content-action-records` 旁路或旧团队快照覆盖完整作战系统。 |
...
| 品牌战情室补素材交付包 | `npm run test:functional -- --test-name-pattern "品牌作战补资源动作会创建审核任务并同步到团队工作区"`，2026-05-30 | 通过。执行队列补素材动作会创建 `material-supplement / needs-material / request-material` 审核任务，并生成 `manifest.json`、`material-gap-list.md`、`material-gap-list.json`；JSON schema 为 `buguai.brand-command.material-gap-list.v1`，清单写入审核任务 ID、缺口行和素材状态，包内不泄漏本机工作区路径。 |
...
| 品牌战情室团队知识包版本边界 | `npm run test:functional -- --test-name-pattern "品牌作战行动记录能追加到团队工作区"`；`npm run test:functional -- --test-name-pattern "品牌作战不会把其他内容知识地图的团队知识包误绑定到队列产物"`；`npm run test:functional -- --test-name-pattern "品牌作战 launch-sop-run 动作会创建真实 SOP 运行并回填资源包"`，2026-05-31 | 通过。品牌战情室直接生成 Prompt 草稿和 SOP 运行时绑定当前地图团队知识包版本，并写入行动记录；只有其他项目 release 时不绑定错误团队知识包。 |
...
| 品牌战情室行动记录导出 | `npm run test:functional -- --test-name-pattern "品牌战情室行动记录导出会生成本机交付文件并同步团队记录"`，2026-05-30 | 通过。导出会生成 `manifest.json`、`action-records.md` 和 `action-records.json`，包内不写本机工作区路径，并追加 `export-action-records` 团队行动记录。 |
| 品牌战情室主动作确认 | `npm run test:functional -- --test-name-pattern "品牌战情室主动作确认会写入行动记录并同步团队事实源"`，2026-05-30 | 通过。目标树、作战编组和执行队列主动作分别写入 `confirm-objectives`、`confirm-resource-bundles` 和 `sync-execution-queue`，执行队列会同步队列条目的团队状态。 |
| Bugu 作战主动作服务端保真 | `/Users/coso/Documents/dev/ai/bugu/bugu`：`npm run smoke:oem-service`、`npm run typecheck`，2026-05-30 | 通过。Bugu `content-action-records` 可保存、分页筛选并返回 `confirm-objectives`、`confirm-resource-bundles` 和 `sync-execution-queue`；控制台把这些动作显示为“确认目标优先级 / 保存作战单元 / 同步执行队列”，不再降级为泛化“内容动作”。 |
...
| 团队共享版本 / current 主事实源 / 业务流同清单和同交付物门禁 | `npm run test:functional -- --test-name-pattern "团队知识包在线验收脚本会分页查找指定 release"`；`npm run test:functional -- --test-name-pattern "团队知识包在线验收要求生产公开包具备大小和 sha256"`；`npm run test:functional -- --test-name-pattern "团队知识包在线验收会拒绝非公网公开包地址"`；`npm run test:functional -- --test-name-pattern "团队共享在线验收要求两个账号看到同一工作区、团队知识包和交付物引用"`；`npm run test:functional -- --test-name-pattern "团队共享在线验收会拒绝空的团队主事实源清单"`；`npm run test:functional -- --test-name-pattern "团队共享在线验收会拒绝空的团队审核任务和执行队列"`；`npm run test:functional -- --test-name-pattern "Ontology v1 在线验收可以汇总知识包和团队共享报告"`；`npm run test:functional -- --test-name-pattern "Ontology v1 生产验收报告归档会拒绝本地 mock 报告"`，2026-06-01 | 通过。`content:release:verify-online` 按分页查找指定 release，不只验证第一页；`content:v1:verify-online --require-public-package` 会把公开包要求传给 release 验收，要求上传状态为 `stored`、包大小大于 0、sha256 为 64 位十六进制、公开包地址为 http/https 公网地址，并拒绝 `file://`、相对路径、本机地址、内网 IP、链路本地地址、IPv6 ULA / link-local 和 IPv4-mapped loopback；`content:team:verify-online` 按分页完整拉取并比对 `content-knowledge-maps`、`content-build-runs`、`content-command-centers` 三类 current 主事实源的非空清单，以及非空 `releaseCount` / `releaseIds` / `releaseListComplete`、`reviewTaskCount` / `reviewTaskIds`、`executionQueueCount` / `executionQueueIds`、`actionRecordCount` / `actionRecordIds`；同时要求两账号看到相同 `actionArtifactRecordIds` 和 `actionArtifactRefsByRecordId`；交付物引用不能包含本机绝对路径、`file://` 或疑似凭证，并且必须包含 `material-gap-list.json`。`content:v1:verify-report -- --production` 拒绝团队知识包版本为空（`team-release-present`）、版本数量 / ID 清单 / 完整拉取标记不一致，以及三类 current 主事实源为空、审核任务为空（`team-review-present`）、执行队列为空（`team-queue-present`）、行动记录为空（`team-action-present`）、交付物引用缺失或不一致、交付物引用不安全、补素材清单缺失、公开包 `publicUrl` 不是 http/https 公网地址、API base 不是公网地址、ID 清单数量少于 count、清单未完整拉取，以及生产模式下跳过 release 或 team 任一顶层验收段（`skipped-section`）的生产报告；schema 已把团队知识包版本摘要、审核任务摘要、执行队列摘要、行动记录摘要、交付物引用摘要、公开包 URL 形态和完整拉取标记列为必需字段。 |
| Bugu 团队清单分页保真 | `/Users/coso/Documents/dev/ai/bugu/bugu`：`npm run smoke:oem-service`，2026-05-30 | 通过。Bugu `content-review-tasks`、`content-action-records` 和 `content-execution-queue` 均返回 `limit / offset / total`，smoke 覆盖 `offset=1` 分页，避免生产验收脚本只验证第一页。 |
...
| AC-09 | 品牌内容作战系统 | Local Verified | `BrandCommandCenterApplicationService`、执行 policy、资源包、队列、行动记录、目标树和详情视图；功能测试覆盖 Bugu 行动记录拉取、权限拦截、未审核资源包待审核、历史 ready 队列审核二次拦截、主动作确认、队列同步、补素材交付包、复盘回写下一轮信号 / 复盘目标 / 资源包 / 作战单元 / 补资源队列、current 快照刷新不覆盖本机已同步更新快照，以及品牌战情室直出 Prompt / SOP 的团队知识包版本绑定和跨地图错绑防护；目标 E2E 覆盖目标树 / 作战编组 / 执行队列主动作，以及审核通过后在执行队列页面点击交接，并验证真实 Prompt 草稿、真实场景卡、真实 SOP 运行、素材覆盖回写、队列行动记录、复盘补素材动作、`handed-off` / `written-back` / `needs-resource` 状态和 Bugu `content-command-centers` 主事实源快照。 | 真实团队战情室、多席位、跨设备行动记录验收。 |
...
| AC-13 | 普通用户不感知 Ontology | Local Verified | `v2-ux-copy-audit` 扫描内容知识地图、审核台、品牌战情室、Prompt 工作台和 SOP 执行页；readiness 检查覆盖范围；prototype 禁用工程词扫描未命中；业务 UI 契约已落文档。 | 真实用户验收仍需确认文案理解成本。 |
...
| 两真实账号 | actor A / B 的工作区、默认 release、审核任务 ID、执行队列 ID、行动记录 ID 一致。 | 缺失。 |
