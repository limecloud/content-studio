...
- Bugu 业务后端是团队内容项目、审核、冲突、发布和业务审计事实源；当前第一刀已覆盖团队工作区、变更包、审核任务、审核结论、执行队列、行动记录、素材覆盖和知识包版本。
...
- Content Studio 的 Prompt 草稿和 SOP 运行记录已能保存团队知识包版本引用，团队成员可以追溯下游内容使用的是哪一版已审核口径。
...
| 团队事实源 | Bugu 保存团队审核后的草稿、审核记录、行动记录和素材覆盖。 | 通过服务端 revision、业务角色和冲突队列合并。 |
...
当前第一刀：Content Studio 桌面端和 Bugu 控制台已生成“合并处理清单”，逐项展示本机提交、团队当前内容、建议处理方式和下一步；Bugu 服务端会把清单保存到冲突记录和行动记录，并推进团队 revision。当前仍不自动覆盖业务内容；素材覆盖的字段级补充只会进入待确认审核任务。
...
- Content Studio 桌面端可查看旧版本提交影响的内容项、版本差异、合并处理清单和处理建议，并选择“保留团队内容 / 重新提交本机修改 / 按清单转人工确认”；服务端保留处理方向和审计记录。
...
- Content Studio 品牌战情室提供“同步团队记录”辅助动作；普通用户可以看到团队同步状态、团队行动记录、资源包交接结果和被拦截原因，不需要理解服务端 action record 结构。
...
- 服务端后续补齐更细字段级确认和审批配置，不能静默覆盖服务端 revision。
- Content Studio 已提供 `content:v1:verify-online` 在线验收总入口，可汇总两账号共享和团队知识包下载验收，并支持 `--output=...` 归档 JSON 报告；两账号必须看到同一批审核任务和同一批执行队列。
- Content Studio 已提供 `content:v1:verify-report` 归档门禁，生产报告必须证明真实 API、真实团队账号、公开包地址、size、sha256、审核任务同清单和执行队列同清单；本地 mock 报告不能作为完成证据。
...
- 私有客户资料、未公开产品资料和内部投放数据必须标记 sensitivity。
- 导出前显示敏感数据检查结果。
- 团队共享源要支持只读消费和写入权限区分。
...
| T3 | 桌面端同步。 | 支持提交、审核、执行队列、行动记录、素材覆盖和知识包版本同步；已补团队行动记录只读刷新和生产交接回填；更完整的全量拉取、离线重试继续补齐。 |
| T4 | 控制台团队视图。 | 已完成工作区面板、同步冲突查看、人工处理结论记录、知识包可分发状态、默认版本回滚、待确认版本批准、多步骤确认进度展示和工作区默认确认模板切换；更细权限配置继续补齐。 |
| T5 | Release 管理。 | 第一刀已能将审核 revision 发布为 Agent Knowledge v0.7.2 包并登记对象 key / 下载地址；控制台可回滚默认版本；待确认版本需负责人批准后成为默认版本；服务端支持多步骤确认和默认确认模板；桌面端可拉取团队版本并缓存；Prompt 草稿和 SOP 运行记录可追溯团队版本；只读在线验收总入口已能校验公开包地址、大小、sha256 和两账号可见性，并能输出 JSON 报告；真实生产环境执行报告继续补齐。 |
...
- 能用只读验收脚本验证两账号看到的审核任务 ID 清单和执行队列 ID 清单一致。
