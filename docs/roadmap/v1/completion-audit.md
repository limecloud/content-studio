...
2. 文字主链必须走协议化 生成服务：Claude 官方链路用 `@anthropic-ai/claude-agent-sdk`，Anthropic / OpenAI / Gemini 兼容网关用对应 HTTP 协议；不可用时明确认证 blocked，不再用本地模板伪造成功。
...
| 文档同步 | `README.md`、`RELEASE_NOTES.md`、`AGENTS.md`、`docs/roadmap/v1/implementation-plan.md` | 本文件和上述文档均说明 Claude SDK、生成服务路由、OpenAI / Gemini / 兼容网关、Generic HTTP 视频拆解 / 生成、blocked 边界 | 完成 | 发布前还需按版本 tag 更新最终 release 文案 |
...
2. 文字、图片、视频拆解、视频生成均有真实生成服务路径和服务级功能测试；OpenAI / Gemini 不再依赖 Claude SDK 运行底座。
