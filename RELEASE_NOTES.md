...
v0.6.0 把知识库和 skills 两个工作台的界面收束成更清晰的 tab 管理与广场式布局，去掉顶部冗余区块，减少一屏塞满状态的感觉。

### 新增 / 调整

- 知识库页改为四个 tab 管理：知识库、引用检索、提示词包、场景卡。
- 移除知识库页顶部 hero / 统计块，把“导入知识库”和“生成场景卡”动作下沉到对应 tab。
- skills 页重做为技能广场布局：顶部横幅、分类筛选、搜索 / 排序、官方精选和其他技能分区。
- skills 卡片改为更清晰的信息层级，支持内置 / 用户安装筛选和更直观的启用、安装、详情操作。
- 清理了全局顶部多余流程展示，让主界面更像工作台而不是流程海报。

### 工程与验证

- 更新版本号到 `0.6.0`，同步 `package.json` 和 `package-lock.json`。
- 更新 Electron smoke 与 E2E 相关断言，避免依赖已移除的顶部区块和旧流程文案。
