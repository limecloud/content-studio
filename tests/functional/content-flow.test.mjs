...
test('Ontology v1 readiness gate 区分本地就绪和生产报告缺失', async () => {
  ...
  assert.ok(localReadiness.checks.some((check) => (
	    ...
	    && check.files === 1
	    && check.rules === 2
	    && check.message.includes('Prompt 工作台')
	  )));
...
test('生产交接会把团队知识包版本绑定到 Prompt 草稿', async () => {
  await withWorkspace(async (workspacePath) => {
    ...
    assert.match(result.record?.actionRecords[0].nextStep ?? '', /Prompt 草稿/);
...
test('embedded 平台宿主连接会复用并发 ensureConnected 初始化', async () => {
  await withEnv({
    CONTENT_STUDIO_DISABLE_EMBEDDED_PLATFORM_HOST: '1',
    LIME_RUNTIME_BRIDGE: undefined,
    LIME_HOST_SNAPSHOT: undefined,
    LIME_DESKTOP_PLATFORM_BRIDGE_DISCOVERY_PATH: join(tmpdir(), 'content-studio-missing-discovery.json'),
  }, async () => {
    const platformHost = new PlatformHostBridgeClient();
    const snapshot = {
      hostKind: 'electron',
      ...
      locale: 'zh-CN',
      theme: 'system',
      modelSettingsVersion: 'concurrent-test',
    };
    let connectCalls = 0;
    let inFlight = 0;
    let maxInFlight = 0;
    platformHost.embeddedHost = {
      ...
      status() {
        return {
          ...
          source: 'embedded',
          snapshot,
          appServerSidecar: { ok: true, connected: true },
        };
      },
      async readSnapshot() {
        return snapshot;
    ...
    assert.equal(platformHost.status().source, 'embedded');
    assert.equal(platformHost.status().appServerSidecar?.ok, true);
  });
});
...
test('平台宿主下未接入平台视觉理解生成服务的视觉/视频直连 Provider 不读取 Product App Key', async () => {
  await withWorkspace(async (workspacePath) => {
    const logs = new GenerationLogStore();
    const inputSources = new InputSourceStore();
    const promptDrafts = new PromptDraftStore(inputSources, new FakeTextGenerationService());
...
test('本地总闸包含 v2 provider dry-run 和业务验收入口', async () => {
  ...
  assert.equal(packageJson.scripts['verify:lime-agent'], undefined);
  ...
  assert.doesNotMatch(packageJson.scripts['verify:local'], /npm run verify:lime-agent/);
  assert.match(packageJson.scripts['verify:local'], /npm run test:functional/);
});
