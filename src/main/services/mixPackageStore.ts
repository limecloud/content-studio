...
function buildImportGuide(record: MixPackageRecord): string {
  ...
  const lines = [
    ...
    '## 交付文件',
    '',
    '- `manifest.json`：完整机器可读清单，保留 SOP、Prompt、来源和审核状态。',
    '- `manifest.csv`：给剪辑人员或第三方混剪软件导入 / 对表使用的简表。',
    '- `images/`：已通过审核的图片素材。',
    ...
    '3. 对绿幕图使用色度抠除 / 透明叠加；如果软件不支持批量读取 JSON，使用 `manifest.csv` 对照素材用途和 Prompt 来源。',
    '4. 发布前复核人工审核状态、授权边界、产品事实和平台禁用表达。',
    '',
    '## 交付边界',
    '',
    '- 本软件只导出素材文件夹和 manifest，不做时间线剪辑、成片渲染或自动发布。',
    '- 第三方视频生成和第三方混剪过程不进入本软件任务状态。',
    '- 如需替换素材，先在素材库重新审核通过后再导出新的混剪包。',
    '',
    '## 素材清单',
    '',
    '| # | 类型 | 标题 | 包内路径 | 审核 | 时长 | Prompt |',
    '| --- | --- | --- | --- | --- | --- | --- |',
    ...assetRows,
  ];
  if (record.notes) {
...
function buildImportCheckMarkdown(record: MixPackageRecord, evidence: MixPackageExternalImportEvidence): string {
  return [
    ...
    `manifest 已导入：${evidence.manifestImported ? '是' : '否'}`,
