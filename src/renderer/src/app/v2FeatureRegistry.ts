...
export const V2_FEATURES: Record<V2ModuleKey, V2FeatureSpec> = {
  ...
  'video-import': {
    eyebrow: '视频 / 入库',
    title: '成品视频导入',
    description: '第三方生成后由用户手动导入视频文件，并选择关联原提示词、图片素材和 SOP 运行记录。',
    ...
    cards: [
      { title: '导入字段', text: '不要求第三方任务编号，只记录本地文件和本软件内的提示词来源。', items: ['文件路径', '提示词来源', '输入素材'] },
  ...
  'video-mix-export': {
    ...
    cards: [
      ...
      { title: 'manifest 字段', text: '包含素材用途、画幅、时长、文案、平台、提示词来源和运行记录。', items: ['素材 ID', '出现时长', '提示词来源'] },
    ],
  ...
  'video-custom': {
    eyebrow: '视频 / 自定义',
    title: '自定义视频',
    description: '高级用户配置时长、画幅、镜头段落、参考图、参考视频和生成服务，普通用户仍走表单化 Prompt。',
    scope: '自定义镜头表单 -> 视频 Prompt -> 复制或内部执行',
    status: 'Prompt 入口',
    primaryAction: '保存视频配置',
    secondaryAction: '生成 Prompt',
    flow: ['填写字段', '配置镜头', '选择生成服务', '生成 Prompt', '执行 / 复制', '记录'],
    cards: [
      { title: '字段配置', text: '不把节点图暴露给普通用户，用表单控制镜头、动作、时长和输出。', items: ['时长', '画幅', '镜头段落'] },
      { title: '生成服务状态', text: '未配置真实服务时只保留待配置记录，不伪造成果。', items: ['内部生成', '手动复制', '待配置'] },
    ],
    preview: '镜头 1：产品特写 3s；镜头 2：手部动作 6s；镜头 3：场景收束 6s。',
    table: [['镜头段', '5 段', '完整', '可运行'], ['生成服务', '未配置', '内部生成', '待配置'], ['外部复制', '可用', '手动', '推荐']],
  },
  'article-title': {
    eyebrow: '文案 / 标题',
    title: '标题生成',
  ...
  'assets-history': {
    ...
    preview: 'run_20260520_001：图片生成服务未配置，图片生成步骤待配置；已保存 Prompt 和输入源，可在配置后重试。',
    table: [['run_001', '待配置', '图片生成服务未配置', '可重试'], ['run_002', '完成', '4 张图片', '可复用'], ['run_003', '待审核', '视频 Prompt', '查看详情']],
  },
  'workflow-definition': {
    eyebrow: '管理 / SOP 定义',
    title: '工作流定义',
    ...
    cards: [
      { title: '定义结构', text: '工作流事实源是可执行定义，不是画布坐标。', items: ['输入字段', '执行步骤', '产物规则'] },
      { title: '上线规则', text: '草案不能直接上线，必须人工确认步骤、生成服务、审核和导出规则。', items: ['草案', '审核', '发布'] },
    ],
